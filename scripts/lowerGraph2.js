...
var chart = Highcharts.chart("container5", {
  ...
  title: {
    text: "Visitors location, geo presentation",
  },
  subtitle: {
    text: "Comparing to results from Summer 2012",
  },
  plotOptions: {
    series: {
      grouping: false,
  ...
  tooltip: {
    ...
    pointFormat:
      '<span style="color:{point.color}">\u25CF</span> {series.name}: <b>{point.y} visits</b><br/>',
  },
  xAxis: {
    type: "category",
    max: 4,
...
years.forEach(function (year) {
  ...
  btn.addEventListener("click", function () {
    ...
    chart.update(
      {
        title: {
          text: "Summer " + year + " - Top 5 countries by highest visits",
        },
        subtitle: {
          text:
            "Comparing to results from Summer" +
            (year - 4) +
